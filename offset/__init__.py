...
def set_node(username, password, identity_file, hostname, etcd, name, purpose):
    with open(path.join(path.dirname(resource_filename(modules[__name__].__package__, '__init__.py')),
                        '_config', 'new_node.json')) as f:
        d = load(f)

    d['name'] = d['uuid'] = d['id'] = d['extra']['ssh_config']['Host'] = name
    d['public_ips'] = d['private_ips'] = d['extra']['ssh_config']['HostName'] = [hostname]
    d['extra']['user'] = d['extra']['ssh_config']['User'] = username
    d['extra']['user_hostname_port'] = '{user}@{hostname}:22'.format(user=username, hostname=hostname)
    if identity_file:
